...
function togglePlayer(id, group) {
  ...
  if (group === 'own') {
    state.optimalLineup = null;
    state.predictionOptimalLineup = null;
  } else if (state.actualOpponentIds.length === 4) {
    const actualPrediction = [{ playerIds: [...state.actualOpponentIds], probability: 1 }];
    state.optimalLineup = buildOptimalOwnLineup(state.ownPlayerIds.map(player), team(state.opponentTeamId).players.map(player), actualPrediction);
  } else if (state.predictionOptimalLineup) {
    state.optimalLineup = state.predictionOptimalLineup;
  }
  render();
}
function optimalHtml() {
  if (state.loading) return '<div class="empty">Analysiere mögliche Gegner-Szenarien und berechne die beste eigene Aufstellung …</div>';
