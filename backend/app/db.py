...
def _database_url() -> str:
    url = os.getenv("DATABASE_URL", "postgresql+psycopg://tt:tt_dev@localhost:5432/tt_aufstellung")
    ...
    if engine.dialect.name == "postgresql":
        with engine.begin() as connection:
            ...
            exists = connection.execute(text(
                "SELECT 1 FROM pg_constraint WHERE conname = 'uq_match_player' AND conrelid = 'match_players'::regclass"
            )).scalar()
            if not exists:
