...
def _upsert_player_master(session, external_player_id: str | None, name: str, club: str | None, observed_at: datetime) -> None:
    """Create/update the XTTV player master from a parsed match player."""
...
def import_one(meid: int) -> dict:
    ...
    with SessionLocal.begin() as session:
        ...
        # XTTV player master data is derived from every successfully imported match.
        # The external_player_id is the XTTV pass/player number and is our stable key.
        ...
        for player in parsed["players"]:
            ...
            club = parsed.get("home_team") if player.get("side") == "home" else parsed.get("away_team")
            _upsert_player_master(session, player.get("external_player_id"), player["name"], club, observed_at)
...
def scan_and_import(start: int, end: int, limit: int = DEFAULT_LIMIT, delay: float = 0.05) -> dict:
    ...
    for meid in _scan_order(start, end, reference):
        ...
        if not _is_valid_4_player_report(parsed):
            errors += 1
            if len(error_samples) < 20:
                error_samples.append({"meid": meid, "error": f"invalid_4_player_report players={parsed.get('player_count')} singles={parsed.get('singles_count')} doubles={parsed.get('doubles_count')}"})
            continue
